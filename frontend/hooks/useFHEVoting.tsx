...
export function useFHEVoting(parameters: {
  ...
  const [proposals, setProposals] = useState<Array<{ id: number; creator: string; description: string }>>([]);
  ...
  const refreshProposals = useCallback(async () => {
    ...
    const items: Array<{ id: number; creator: string; description: string }> = [];
    for (let i = 0; i < Number(count); i++) {
      const [creator, description]: [string, string] = await c.getProposal(i);
      items.push({ id: i, creator, description });
    }
    setProposals(items);
  ...
  const refreshMyVotes = useCallback(async () => {
    ...
    // Try typed filter first; fallback to raw topics if unavailable
    let logs: ethers.Log[] = [];
    try {
      ...
      const event = iface.getEvent("Voted");
      const topic0 = iface.getEventTopic(event);
  ...
  const createProposal = useCallback(async (description: string) => {
    ...
    try {
      ...
      const tx = await c.createProposal(description);
      await tx.wait();
  ...
  }, [voting.address, voting.abi, ethersSigner, instance, refreshTallies]);
  ...
  const decryptTallies = useCallback(async (proposalId: number) => {
    ...
    if (t?.yes) toDecrypt.push({ handle: t.yes as EncryptedHandle, contractAddress: voting.address });
    if (t?.no) toDecrypt.push({ handle: t.no as EncryptedHandle, contractAddress: voting.address });
    ...
    setClearTallies((prev) => ({
      ...prev,
      [proposalId]: {
        yes: t?.yes ? { handle: t.yes as EncryptedHandle, clear: res[t.yes as EncryptedHandle] } : prev[proposalId]?.yes,
        no: t?.no ? { handle: t.no as EncryptedHandle, clear: res[t.no as EncryptedHandle] } : prev[proposalId]?.no,
      },
    }));
