...
const CONTRACTS = ["FHECounter", "FHEVoting"]; // Generate both
...
for (const CONTRACT_NAME of CONTRACTS) {
  ...
  if (deployLocalhost && deploySepolia) {
    if (
      ...
      console.error(
        `${line}Deployments for ${CONTRACT_NAME} on localhost and Sepolia differ. Cant use the same abi on both networks. Consider re-deploying the contracts on both networks.${line}`
      );
      process.exit(1);
    }
  }
